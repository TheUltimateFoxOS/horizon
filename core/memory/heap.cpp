...
heap_segment_header_t* heap_segment_header_t::split(size_t split_length) {
	...
	int64_t split_seg_length = this->length - (split_length + sizeof(heap_segment_header_t));
	...
	if (this->next) {
		...
		new_split_hdr->next = this->next; // Set the new segment's next segment to the next segment
	} else {
		new_split_hdr->next = NULL; // Set the new segment's next segment to out original next segment
	}
	this->next = new_split_hdr; // Set our new segment to the new segment
	...
	new_split_hdr->free = this->free; // make sure the new segment's free is the same as the original
	this->length = split_length; // set the length of the original segment to its new length
...
void heap_segment_header_t::combine_forward() {
	...
	this->length = this->length + this->next->length + sizeof(heap_segment_header_t);
